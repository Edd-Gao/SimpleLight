...
(function($) {
	...
	methods.staticInit = function() {
		...
		//$('body').on('touchmove.wheelColorPicker', private.onBodyTouchMove);
		document.getElementById('canvas').addEventListener("touchmove",private.onBodyTouchMove,false);
		//$('body').on('touchend.wheelColorPicker', private.onBodyTouchEnd);
		document.getElementById('canvas').addEventListener("touchend",private.onBodyTouchEnd,false);
	};
	...
	private.initWidget = function( widget ) {
			...
			"<div class='jQWCP-wWidget'>" + 
				...
				"<div class='jQWCP-wHSVGroup'>" +
					...
					"<div class='jQWCP-wVal jQWCP-slider-wrapper'>" +
						...
						"<span class='jQWCP-wValCursor jQWCP-scursor'><img src='css/images/slider.png' class='jQWCP-scursor-img' /></span>" +
					"</div>" +
				"</div>" +
		...
		$widget.on('touchstart.wheelColorPicker', '.jQWCP-wWheel', private.onWheelTouchStart);
		$widget.on('touchstart.wheelColorPicker', '.jQWCP-wWheelCursor', private.onWheelCursorTouchStart);
		$widget.on('touchstart.wheelColorPicker', '.jQWCP-slider', private.onSliderTouchStart);
		$widget.on('touchstart.wheelColorPicker', '.jQWCP-scursor', private.onSliderCursorTouchStart);
	...
	private.onWheelMouseDown = function( e ) {
		...
		console.log($('body').data('jQWCP.activeControl', $this.get(0)));
	...
	/**
	 * Function: onWheelTouchStart
	 * 
	 * Begin clicking the wheel down. This will allow user to move 
	 * the crosshair although the mouse is outside the wheel.
	 */
	private.onWheelTouchStart = function( e ) {
		var $this = $(this); // Refers to wheel
		var $widget = $this.parents('.jQWCP-wWidget:eq(0)');
		var $input = $( $widget.data('jQWCP.inputElm') );
	 ...
	 * the crosshair although the mouse is outside the wheel.
	 */
	private.onWheelCursorTouchStart = function( e ) {
		var $this = $(this); // Refers to cursor
		var $widget = $this.parents('.jQWCP-wWidget:eq(0)');
		var $input = $( $widget.data('jQWCP.inputElm') );
	 ...
	 * the slider although the mouse is outside the slider.
	 */
	private.onSliderTouchStart = function( e ) {
		var $this = $(this); // Refers to slider
		var $widget = $this.parents('.jQWCP-wWidget:eq(0)');
		var $input = $( $widget.data('jQWCP.inputElm') );
	 ...
	 * the slider although the mouse is outside the slider.
	 */
	private.onSliderCursorTouchStart = function( e ) {
		var $this = $(this); // Refers to slider cursor
		var $widget = $this.parents('.jQWCP-wWidget:eq(0)');
		var $input = $( $widget.data('jQWCP.inputElm') );
	 ...
	 * 
	 * Move the active slider (when touch is started).
	 */
	private.onBodyTouchMove = function( e ) {
		var $control = $( $('body').data('jQWCP.activeControl') ); // Refers to slider wrapper or wheel
		console.log("Touch move: ", $control);
		e.preventDefault();//prevent the page from draging away
		// Do stuffs when popup is open
		if($control.length == 0)
			return;
	 ...
	 *Clear active control reference.
	 */
	 private.onBodyTouchEnd = function( e ) {
		var $control = $( $('body').data('jQWCP.activeControl') ); // Refers to slider wrapper or wheel
		console.log("Touch end: ", $control);
		if($control.length != 0) {
			var $widget = $control.parents('.jQWCP-wWidget:eq(0)');
			var $input = $( $widget.data('jQWCP.inputElm') );
			var settings = $input.data('jQWCP.settings');
	...
	private.updateActiveControl = function( e ) {
		...
		if($control.hasClass('jQWCP-wWheel')) {
			...
			var relX = ($pageX - $control.offset().left - ($control.width() / 2)) / ($control.width() / 2);
			var relY = - ($pageY - $control.offset().top - ($control.height() / 2)) / ($control.height() / 2);
		...
		/// SLIDER CONTROL ///
			...
			var relY = ($pageY - $control.offset().top) / $control.height();
